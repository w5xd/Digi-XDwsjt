! modified by Wayne Wright, W5XD to fix a bug.
! The bug is:
!      this routine overwrites either cwave or wave depending on whether icmplx is nonzero or zero, respectively.
!      But the original code writes to wave regardless. Callers that fail to point wave (paramter 8) to
!       an array large enough get their memory gratuituously zeroed....
!
! The Digi-XDwsjt Makefile gives priority to finding a source file here over the original wsjtx source
...
! Calculate and insert the audio waveform
  ...
  if(icmplx .ne. 0) then  !Avoid writing to memory we may not have access to
          cwave=0. 
  else 
          wave=0.
  endif
