...
!
! This implementation of the ft8_decode routine does not attempt to do what WSJT-X 2.6 does, 
! which is to save the results of early decodes for use later in the cycle. In principle,
! that is at best a performance enhancement. Why? Because when this decoder is invoked later
! in the cycle, it knows all the incoming data all over again, and then more. Also in principle
! the only decoding advantage possible for early decodes would be if the decoding algorithm takes
! the shorter length of the incoming sound buffer into account.
module ft8_decode
